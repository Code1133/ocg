 ...
 */
static constexpr uint32 GExpectedCRC = 0x128B9D93;

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FOCGGolden_WithRiver,
	"OCG.Regression.WithRiver",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)

bool FOCGGolden_WithRiver::RunTest(const FString& Parameters)
{
	UMapPreset* Preset = TryLoadPreset(this, TEXT("/OneButtonLevelGeneration/Test/MP_Golden_WithRiver.MP_Golden_WithRiver"));
	if (!Preset)
	{
	...
	const FOCGBenchmarkEntry Entry = FOCGBenchmarkRunner::Run(FName(TEXT("WithRiver")), [&]
	{
		DataGen->GenerateData(Preset);
	});
	FOCGBenchmarkRunner::AppendToCSV(Entry);

	return CheckHeightMapCRC(this, DataGen->GetDataContainer().HeightMapData, GExpectedCRC, TEXT("WithRiver"));
}
