 ...
 */
static constexpr uint32 GExpectedCRC = 0x2503D079;
...
bool FOCGGolden_NoIsland::RunTest(const FString& Parameters)
{
	...
	const FOCGBenchmarkEntry Entry = FOCGBenchmarkRunner::Run(FName(TEXT("NoIsland")), [&]
	{
		DataGen->GenerateData(Preset);
	});
	FOCGBenchmarkRunner::AppendToCSV(Entry);

	return CheckHeightMapCRC(this, DataGen->GetDataContainer().HeightMapData, GExpectedCRC, TEXT("NoIsland"));
}
